...
extern void Configura_Reg_PWM0(uint16_t freq)
{
    ...
    //    Gen
    PWM0->_0_CTL = (0<<0)|(0<<1);//se le asigna 0 a 0 porque es el bit para indicar lo habilitado o desabilitado según pag. 1708
    PWM0->_1_CTL = (0<<0);
    PWM0->_2_CTL = (0<<0);
   //PWM0->_0_CTL = (0<<1);
    
    //pag. 1687 para saber con que comparador trabaja el PWM a usar
    // Modo de configuración - modo regresivo pag. 1727, pag. 1678 para saber que valor poner, y 1248 para saber con que comparador trabaja el PWM a usar 
   PWM0->_0_GENB |= (0X2<<2)|(0X3<<10)|(0x0<<0); ///SIII -YA NO
   //PWM0->_0_GENB |= (0X2<<2)|(0X3<<6)|(0x0<<0);
   //PWM0->_0_GENB = 0x0000080C;     
   PWM0->_1_GENA |= (0X2<<2)|(0X3<<6)|(0x0<<0); //SIII 
   // PWM0->_1_GENA = 0x0000008C;
   // PWM0->_2_GENA = 0x0000008C;
   PWM0->_2_GENA |= (0X2<<2)|(0X3<<6)|(0x0<<0); //SIII
    //prueba
   // PWM0->_2_GENA |= (0X2<<2)|(0X3<<6)|(0x0<<0); //1711
   // PWM0->_1_GENB |= (0X2<<2)|(0X3<<10)|(0x0<<0); |(0x2<<6)
    //PWM0->_0_GENB |= (0x2<<10)|(0x3<<2)|(0x0<<0);
    ...
    PWM0->_0_CMPB = 5000;//45000; // SII -NO JUNTOS ciclo de trabajo 70%, los primeras 3500 cuentas estaran en alto, y despues va a bajar
     // PWM0->_0_CMPB = 2000;
     // PWM0->_0_CMPA = 45000;
      PWM0->_1_CMPA = 5000;           
      PWM0->_2_CMPA = 5000;
