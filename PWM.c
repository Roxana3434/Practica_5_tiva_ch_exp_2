...
extern void Configura_Reg_PWM1(uint16_t freq)
{
    ...
    //SYSCTL->RCC |=(1<<20)|(0<<19)|(1<<18)|(0<<17); //Se Habilita porque salen 50 mil cuentas que caben en los 16 bits del contador del PWM, 20 millinoes/ 50 = 400 mil cuentas  
    ...
    /*Se habilita la función alternativa para pines Pag 672*/
    GPIOB->AFSEL |= (1<<4); //PB4
    GPIOB->AFSEL |= (1<<7); //PB7
    GPIOE->AFSEL |= (1<<4); //PE4

    //Se habilita como salida los pines del pwm 
    GPIOB->DIR = (1<<4); //PB4
    GPIOB->DIR = (1<<7); //PB7
    GPIOE->DIR = (1<<4); //PE4
    
    /*Indicar func. alternativa: Combinado con la tabla Pag 1351 y el registro PCTL le digo que es pwm Pag 689, por eso se pone el valor de 4 en el pin */
    ...
    PWM0->_1_CMPA = 37500; //Para 25% en ciclo de trabajo 75% de las 50000 cuentas, Gen 1 trabaja con comparador A por pag. 1248 
