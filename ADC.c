...
extern void Configura_Reg_ADC0(void)
{
    ...
    ADC0->SSMUX1 = (2<<0); //en el secuenciador 0 se asignar el canal 2
    ADC0->SSMUX2 = (8<<0); //en el secuenciador 0 se asignar el canal 8
...
extern void ADC0_InSeq2(uint16_t *Result,uint16_t *duty)
{
       ...
       //canal 1
       Result[0] = ADC0->SSFIFO0&0xFFF; // En el FIFO0 se almacenan las muestras, por lo que se lee, pag 860
       // duty[0] = (Result[0]*5000)/4096; ///2ms
       duty[0] = 50000-(Result[0]*5000)/4096; //pseudo jala 
       // duty[0] = 50000-(Result[0]*2500)/4096;
      // duty[0] =   (Result[0]*3000)/4096; ///cuentas para 1.5 ms 
       ...
       //duty[0] = (Result[0]*20000)/4096; ///1.5ms en cuentas en lugar de 2 ms 
